...
function AddAccountDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleDropboxLogin = async () => {
    ...
    try {
      ...
      let msg = "Gagal memulai login Dropbox. Coba lagi.";
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()} className="max-w-xl">
      <DialogHeader>
        <DialogTitle className="flex items-center gap-2">
          <SiDropbox color={DROPBOX_BLUE} className="h-5 w-5" />
          Tambah Akun Dropbox
        </DialogTitle>
        <DialogDescription>Hubungkan akun Dropbox ke storage pool.</DialogDescription>
      </DialogHeader>
      ...
      <div className="rounded-xl border-2 border-brand-500/40 bg-brand-50/50 p-4 dark:border-brand-500/30 dark:bg-brand-950/20">
        ...
        >
          {oauthUrlMutation.isPending ? (
            <><Loader2 className="h-4 w-4 animate-spin" />Mengalihkan ke Dropbox...</>
          ) : (
            <>
              <SiDropbox color={DROPBOX_BLUE} className="h-5 w-5" />
              Login dengan Dropbox
            </>
          )}
        </button>

        <p className="mt-2.5 text-[11px] leading-relaxed text-[rgb(var(--ink-500))]">
          Kamu akan diarahkan ke halaman izin Dropbox. Setelah menekan <strong>Allow</strong>,
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogHeader>
        <div className="flex items-center gap-3 mb-1">
          ...
          <DialogTitle>Format Dropbox?</DialogTitle>
        </div>
        <DialogDescription className="pl-[52px]">
          <strong className="text-[rgb(var(--foreground))]">Seluruh isi Dropbox</strong> akun{" "}
      ...
      <div className="mx-4 mb-2 rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/30 p-3">
        <p className="text-xs text-red-700 dark:text-red-400 font-medium">
          Tindakan ini tidak bisa dibatalkan. Seluruh file akan hilang selamanya dari Dropbox.
        </p>
      </div>
      ...
      <DialogFooter>
        ...
        <Button variant="destructive" onClick={onConfirm} disabled={!matches || isPending}>
          {isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Memformat...</> : <><HardDrive className="mr-2 h-4 w-4" />Format Dropbox</>}
        </Button>
      </DialogFooter>
    </Dialog>
  );
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogHeader>
        ...
        <DialogDescription className="pl-[52px]">
          ...
          — lalu dihapus dari sumber. File tujuan boleh beda provider (Dropbox / Google Drive).
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="mx-4 mb-2 flex flex-col gap-1.5">
        ...
        <div className="flex flex-col gap-1.5 max-h-44 overflow-y-auto">
          ...
          {candidates.map((account) => {
            ...
            const provIcon = account.provider === "dropbox" ? undefined
              : account.provider === "onedrive" ? onedriveSvg
              : googleDriveSvg;
            return (
              <button key={account.id} type="button" disabled={!selectable}
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-medium text-[rgb(var(--foreground))] truncate">{maskEmail(account.email)}</p>
                    <p className="text-[10px] text-zinc-400 mt-0.5">
                      {account.provider === "dropbox" ? "Dropbox" : account.provider === "onedrive" ? "OneDrive" : "Google Drive"} · Sisa {formatBytes(account.availableStorageBytes)}
                      {!isOnline && " • Offline"}
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      ...
      <div className="mx-4 mb-2 rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30 p-3">
        <p className="text-xs text-amber-700 dark:text-amber-400">
          {fileCount > 0
            ? `${fileCount} file yang tercatat tetap ada di list dashboard, tapi tidak bisa didownload sampai akun ini login ulang dengan Dropbox. File di Dropbox tidak dihapus.`
            : "Akun ini tidak memiliki file yang tercatat dan akan dihapus dari daftar. File di Dropbox tidak dihapus."}
        </p>
      </div>
      <DialogFooter>
...
function DropboxPage() {
  ...
  useEffect(() => {
    ...
    if (oauthStatus === "success") {
      ...
      toast({ title: "Akun Dropbox berhasil terhubung", description: email, variant: "success" });
      ...
      toast({ title: "Gagal menambahkan akun Dropbox", description: reason, variant: "error" });
    }
  ...
  const dropboxAccounts = useMemo(
    () => accountsData?.accounts.filter((acc) => acc.provider === "dropbox") || [],
    [accountsData]
  );
  ...
  const combined = useMemo(() => {
    const total = dropboxAccounts.reduce((s, a) => s + a.totalStorageBytes, 0);
    const used = dropboxAccounts.reduce((s, a) => s + a.usedStorageBytes, 0);
  ...
  const handleFormatConfirm = async () => {
    ...
    try {
      ...
      toast({ title: "Gagal memformat Dropbox", description: error instanceof Error ? error.message : undefined, variant: "error" });
    } finally {
    ...
    <PageTransition>
      <div className="flex h-full flex-col gap-6">
        <PageHeader
          ...
          title="Dropbox"
          description="Kelola storage dan akun Dropbox yang terhubung."
          actions={
            <>
              <button
              ...
              <button
                ...
                aria-label="Add Dropbox"
              >
                <Plus className="h-4 w-4 shrink-0" />
                <span className="hidden sm:inline">Add Dropbox</span>
              </button>
            </>
          }
        />

        {/* ── Combined capacity (akun Dropbox saja) ── */}
        <Card className="shrink-0 relative overflow-hidden border-0 bg-[rgb(var(--surface))] shadow-sm ring-1 ring-zinc-200 dark:ring-white/5">
          <div className="pointer-events-none absolute -top-12 -right-12 h-48 w-48 rounded-full bg-[#0061FF]/15 blur-3xl dark:bg-[#0061FF]/5" />
          <div className="relative z-10 p-5">
            <div className="mb-3 flex items-center gap-2 font-semibold text-brand-700 dark:text-brand-300">
              <Database className="h-5 w-5" />
              Total Kapasitas Dropbox
            </div>
        ...
        <div className="flex-1 flex flex-col min-h-[300px]">
          <h2 className="mb-4 text-lg font-semibold text-[rgb(var(--foreground))]">Akun Dropbox ({dropboxAccounts.length})</h2>
            ...
            {isAccountsLoading ? (
              ...
              <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-[#0061FF]/10">
                  <SiDropbox color={DROPBOX_BLUE} className="h-7 w-7 opacity-70" />
                </div>
                <p className="mt-4 text-sm font-medium text-[rgb(var(--foreground))]">Belum ada akun Dropbox</p>
                <p className="mt-1 text-xs text-[rgb(var(--ink-500))]">Klik tombol "Add Dropbox" di kanan atas untuk mulai.</p>
              </motion.div>
            ) : (
                            ...
                            <div className="flex items-start justify-between p-4 sm:p-5 pb-3">
                              <div className="flex min-w-0 flex-1 items-center gap-3">
                                <Avatar className="h-9 w-9 sm:h-10 sm:w-10 shrink-0 ring-1 ring-zinc-200 dark:ring-zinc-800">
                                  <AvatarFallback className="bg-[#0061FF]/10 font-semibold text-[#0061FF] dark:bg-[#0061FF]/20">
                                    {account.email.charAt(0).toUpperCase()}
                                  </AvatarFallback>
                                </Avatar>
                                <div className="flex min-w-0 flex-col">
                                  <EmailCell email={account.email} />
                                  <span className="text-[11px] text-zinc-500">Dropbox Storage</span>
                                </div>
                              </div>
                            </div>
