...
function PremiumAccountCard({ account, index, onFormat, onMigrate, onDelete, activeJobs }: { account: DriveAccountWithFileCount; index: number; onFormat: () => void; onMigrate: () => void; onDelete: () => void; activeJobs: any[] }) {
  ...
  const isDanger = usagePct > 90; const isWarning = usagePct > 75 && usagePct <= 90; const isSyncing = account.status === "syncing"; 
  const accId = account.accountId ?? "";
  const isMigrating = activeJobs.some((j: any) => {
    const src = j.sourceAccountId ?? "";
    const tgt = j.targetAccountId ?? "";
    return src === account.accountId || tgt === account.accountId || src === accId || tgt === accId;
  }); 
  const provider = (account.provider || "google_drive").toLowerCase(); const badgeCls = PROVIDER_BADGE[provider] || "bg-[rgb(var(--surface-muted))] text-[rgb(var(--ink-500))] ring-[rgb(var(--border-subtle))]";
  return (
...
function StoragePage() {
  ...
  const [formatTarget, setFormatTarget] = useState<{ accountId: string; email: string; fileCount: number } | null>(null);
  const { startMigration, isStarting: isMigrationStarting, activeJobs } = useMigrationGlobal();
  const [migrateSource, setMigrateSource] = useState<{ accountId: string; email: string; fileCount: number; usedBytes: number } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<{ accountId: string; email: string; fileCount: number } | null>(null);
  ...
  const handleDeleteConfirm = async () => { if (!deleteTarget) return; try { await deleteAccount.mutateAsync(deleteTarget.accountId); toast({ title: "Akun diputus", variant: "success" }); } catch (e) { toast({ title: "Gagal", description: e instanceof Error ? e.message : undefined, variant: "error" }); } finally { setDeleteTarget(null); } };
  const handleMigrateConfirm = async (targetId: string) => { if (!migrateSource) return; try { await startMigration(migrateSource.accountId, targetId); toast({ title: "Migrasi dimulai", description: "Pantau progress di icon Send kanan atas.", variant: "success" }); setMigrateSource(null); } catch (e) { toast({ title: "Gagal migrasi", description: e instanceof Error ? e.message : undefined, variant: "error" }); } };
  const handleFormatConfirm = async () => { if (!formatTarget) return; try { const r = await formatDrive.mutateAsync(formatTarget.accountId); toast({ title: `${r.deletedFiles} file dihapus dari ${formatTarget.email}`, variant: "success" }); } catch (e) { toast({ title: "Gagal format", description: e instanceof Error ? e.message : undefined, variant: "error" }); } finally { setFormatTarget(null); } };
          ...
          <AnimatePresence mode="wait">
            {isAccountsLoading ? (<motion.div key="skeleton" variants={containerVariants} initial="hidden" animate="show" exit="hidden"><CardGridSkeleton count={6} /></motion.div>) : filteredAccounts.length === 0 ? (
              ...
              <motion.div key="grid" variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {filteredAccounts.map((acc, idx) => (<PremiumAccountCard key={acc.accountId} account={acc} index={idx} activeJobs={activeJobs} onFormat={() => setFormatTarget({ accountId: acc.accountId, email: acc.email, fileCount: acc.fileCount ?? 0 })} onMigrate={() => setMigrateSource({ accountId: acc.accountId, email: acc.email, fileCount: acc.fileCount ?? 0, usedBytes: acc.usedStorageBytes })} onDelete={() => setDeleteTarget({ accountId: acc.accountId, email: acc.email, fileCount: acc.fileCount ?? 0 })} />))}
              </motion.div>
            ) : (
              <motion.div key="list" variants={containerVariants} initial="hidden" animate="show" className="flex flex-col gap-2">
                {filteredAccounts.map((acc) => {
                  const pct = acc.totalStorageBytes > 0 ? (acc.usedStorageBytes / acc.totalStorageBytes) * 100 : 0;
                  return (
                    <motion.div key={acc.accountId} variants={itemVariants} className={cn(bentoBase, "flex items-center gap-3 p-3 sm:p-4")}>
                      ...
                      <div className="flex items-center gap-1"><button onClick={() => setMigrateSource({ accountId: acc.accountId, email: acc.email, fileCount: acc.fileCount ?? 0, usedBytes: acc.usedStorageBytes })} className="grid h-8 w-8 place-items-center rounded-full hover:bg-[rgb(var(--surface-muted))]"><ArrowLeftRight className="h-4 w-4" /></button><button onClick={() => setFormatTarget({ accountId: acc.accountId, email: acc.email, fileCount: acc.fileCount ?? 0 })} className="grid h-8 w-8 place-items-center rounded-full hover:bg-red-50 hover:text-red-600"><HardDrive className="h-4 w-4" /></button><button onClick={() => setDeleteTarget({ accountId: acc.accountId, email: acc.email, fileCount: acc.fileCount ?? 0 })} className="grid h-8 w-8 place-items-center rounded-full hover:bg-red-50 hover:text-red-600"><Power className="h-4 w-4" /></button></div>
                    </motion.div>
                  );
                })}
              </motion.div>
